...
func IsErrNotFound(err error) bool {
	return err != nil && (err.Error() == "not found" ||
		err.Error() == "repository error: not found" ||
		err.Error() == "object error: not found" ||
		err.Error() == "reference error: not found")
}

// IsErrNotRepo checks if an error indicates "not a repository"
func IsErrNotRepo(err error) bool {
	return err != nil && (err.Error() == "not a repository" ||
		err.Error() == "repository error: not a repository")
}

// NotFoundError creates a standardized "not found" error
func NotFoundError(category, item string) error {
	return NewError(category, fmt.Sprintf("%s not found", item), nil)
}
